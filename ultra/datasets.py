...
class GrailSymmetricrelationDataset(InMemoryDataset):
    ...
    @property
    def raw_dir(self):
        return os.path.join(self.root, "grail", self.name, self.version, "raw","sym")

    @property
    def processed_dir(self):
        return os.path.join(self.root, "grail", self.name, self.version, "processed","sym")
    ...
    def raw_file_names(self):
        return [
            "train_ind_symmetric_relations.txt", "test_symmetric_relations.txt", "train_symmetric_relations.txt", "valid_symmetric_relations.txt"
        ]
    
    '''    
    @property
    def sym_file_names(self):
        return [
            "train_ind_symmetric_relations.txt", "test_symmetric_relations.txt", "train_symmetric_relations.txt", "valid_symmetric_relations.txt"
        ]
    @property
    def tran_file_names(self):
        return [
            "train_q_transitive_relations.txt", "test_q_transitive_relations.txt", "train_q_transitive_relations.txt", "valid_q_transitive_relations.txt"
        ]
    ...
    def process(self):
        test_files = self.raw_paths[:2]  #"train_ind_symmetric_relations.txt", "test_symmetric_relations.txt"
        train_files = self.raw_paths[2:] #"train_symmetric_relations.txt", "valid_symmetric_relations.txt"
        ...
        for txt_file in test_files:
            with open(txt_file, "r") as fin:
                ...
                for line in fin:
                    ...
                    num_sample += 1
            num_samples.append(num_sample)
        triplets = torch.tensor(triplets)  #shape(num_triplets,3)
        '''
        for txt_file in test_files:
            with open(txt_file, "r") as fin:
                num_sample = 0
        ...
        edge_index = triplets[:, :2].t()  #取每个三元组的前两列即头尾实体，转置 shape(2,num_triplets),edge_index的第一行是所有头实体，第二行是所有尾实。代表一个有向图中的所有边。
        edge_type = triplets[:, 2]  #取三元组的第三列，即关系r
        num_relations = int(edge_type.max()) + 1  #索引从0开始，关系数量为最大值+1
        ...
        #训练的事实来自train.txt，测试的事实来自train_ind.txt
        train_fact_slice = slice(None, sum(num_samples[:1])) #num_samples[:1]取列表的第一个元素（训练集文件的样本数,train.txt）,slice(None, sum(num_samples[:1])) 创建了一个从起始位置到训练集train.txt结束位置的切片
        test_fact_slice = slice(sum(num_samples[:2]), sum(num_samples[:3])) #sum(num_samples[:2])前两个文件train.txt, valid.txt；slice(sum(num_samples[:2]), sum(num_samples[:3])) 创建了一个从train_ind.txt起始位置到结束位置的切片。
        ...
        # add flipped triplets for the fact graphs  添加反向边
        ...
        train_slice = slice(None, sum(num_samples[:1]))  #创建了一个从起始位置到训练集train.txt结束位置的切片
        valid_slice = slice(sum(num_samples[:1]), sum(num_samples[:2]))  #valid.txt的切片
        ...
        #如果测试验证合并(erge_valid_test)测试分割为valid_ind.txt + test_ind.txt；不合并则测试分割是test_ind.txt
        ...
        #train_data中边edge来自训练事实(train.txt),节点数为训练集实体数，目标边target_edge，来自训练分割即train.txt
        ...
        #valid_data中边edge来自训练事实(train.txt),节点数为训练集实体数，目标边target_edge，来自验证分割即valid.txt
        ...
        #test_data中边edge来自测试事实(train_ind.txt),节点数为训练集实体数，目标边target_edge，来测试分割即valid_ind.txt + test_ind.txt或者test_ind.txt
...
class GrailTransitiverelationDataset(InMemoryDataset):
    ...
    @property
    def raw_dir(self):
        return os.path.join(self.root, "grail", self.name, self.version, "raw","tran")

    @property
    def processed_dir(self):
        return os.path.join(self.root, "grail", self.name, self.version, "processed","tran")
...
class FB15k237Inductivesym(GrailSymmetricrelationDataset):
...
class FB15k237Inductivetran(GrailTransitiverelationDataset):
...
class WN18RRInductivesym(GrailSymmetricrelationDataset):
    ...
    def __init__(self, root, version):
        super().__init__(root, version)
class WN18RRInductivetran(GrailTransitiverelationDataset):
...
class GrailInductiveDataset(InMemoryDataset):
    ...
    def process(self):
        test_files = self.raw_paths[:3]  #包含 test_ind.txt, valid_ind.txt, train_ind.txt
        train_files = self.raw_paths[3:] #train.txt, valid.txt
...
yago310特殊关系测试
'''
class YAGO310tran(TransductiveDataset):

    name = "yago310_t"

class YAGO310sym(TransductiveDataset):

    name = "yago310_s"
class YAGO310symtran(TransductiveDataset):
