...
def train_and_validate(cfg, model, train_data, valid_data, device, logger, filtered_data=None, batch_per_epoch=None):
    if cfg.train.num_epoch == 0:  #零样本推理，epoch==0，直接返回不进行训练
        return

    world_size = util.get_world_size()
    rank = util.get_rank()

    #train_triplets.shap(num_edges, 3);train_data.target_edge来自train.txt，FB15k237Inductive中为4245个
    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        for epoch in range(i, min(cfg.train.num_epoch, i + step)):
            ...
            for batch in islice(train_loader, batch_per_epoch):
                batch = tasks.negative_sampling(train_data, batch, cfg.task.num_negative,
                                                strict=cfg.task.strict_negative)  #num_negative: 256  严格负采样
                #负采样后更新的batch.shape(batch_size, 1 + num_negative, 3)
                pred = parallel_model(train_data, batch)  #train.txt  经过模型后输出的是尾节点得分
                target = torch.zeros_like(pred) #构造一个与pred 相同形状的张量，初始值全为0
                target[:, 0] = 1  #每行的第一个元素设为1（正样本）
                loss = F.binary_cross_entropy_with_logits(pred, target, reduction="none") #二元交叉熵损失
                neg_weight = torch.ones_like(pred)  #权重初始设为全1.shape同pred
                if cfg.task.adversarial_temperature > 0: #启用对抗性温度
                    with torch.no_grad():
                        neg_weight[:, 1:] = F.softmax(pred[:, 1:] / cfg.task.adversarial_temperature, dim=-1)
                else:
                    neg_weight[:, 1:] = 1 / cfg.task.num_negative  #均值
        ...
        #filtered_data：train.txt+valid.txt
...
#main函数 先用valid_data执行，filtered_data：train.txt+valid.txt
#后用test_data执行，filtered_data：train_ind.txt+（valid_ind.txt + test_ind.txt）合并验证测试集
def test(cfg, model, test_data, device, logger, filtered_data=None, return_metrics=False):
    world_size = util.get_world_size()
    rank = util.get_rank()
    #test_triplets.shap(num_edges, 3)
    #用valid_data时test_triplets数据来自valid.txt， FB15k237Inductive中489个
    #用test_data时test_triplets数据来自valid_ind.txt + test_ind.txt， FB15k237Inductive中411（206+205）个
    ...
    for batch in test_loader:
        t_batch, h_batch = tasks.all_negative(test_data, batch)  #生成负样本
        t_pred = model(test_data, t_batch)
        h_pred = model(test_data, h_batch)

        if filtered_data is None:  #filtered_data包括test_data，比test_data更全
            t_mask, h_mask = tasks.strict_negative_mask(test_data, batch)  #生成掩码，确保负样本的选择不包含真实的尾节点/头节点
        else:
            t_mask, h_mask = tasks.strict_negative_mask(filtered_data, batch)
        pos_h_index, pos_t_index, pos_r_index = batch.t()  #batch.t()形状(3,num_edges) 取了正样本头/尾/关系索引 每一个shape都为（num_edges,）
        t_ranking = tasks.compute_ranking(t_pred, pos_t_index, t_mask) #pos_t_index目标值
...
if __name__ == "__main__":
    ...
    if task_name == "InductiveInference":
        ...
        # Grail, MTDEA, HM datasets have validation sets based off the training graph 验证集基于训练图
        # ILPC, Ingram have validation sets from the inference graph 验证集基于推理图
        # filtering dataset should contain all true edges (base graph + (valid) + test) 
        if "ILPC" in cfg.dataset['class'] or "Ingram" in cfg.dataset['class']:  #验证集基于推理图
    ...
    if util.get_rank() == 0:    #FB15K-237  filtered_data=val_filtered_data 包括train_data.edge + valid_data.target_edge（train.txt+valid.txt）
        logger.warning(separator)
        logger.warning("Evaluate on valid")
    test(cfg, model, valid_data, filtered_data=val_filtered_data, device=device, logger=logger)
    if util.get_rank() == 0:  #filtered_data=val_filtered_data同train_and_validate
        logger.warning(separator)
        logger.warning("Evaluate on test")
    test(cfg, model, test_data, filtered_data=test_filtered_data, device=device, logger=logger)  
    #FB15K-237 filtered_data=test_filtered_data 包括推理+测试边  test_data.edge + test_data.target_edge  （train_ind.txt+（valid_ind.txt + test_ind.txt））
